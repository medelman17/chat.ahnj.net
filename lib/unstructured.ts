...
export function withNestedChildren(
  elements: UnstructuredElement[],
): UnstructuredElementWithChildren[] {
  ...
  elements.forEach((element) => {
    ...
    if (!parentId) {
      ...
      if (parent) {
        ...
        console.warn(
          `Parent element with ID ${parentId} not found for element ${element.element_id}`,
        );
        rootElements.push(elementWithChildren);
      }
    }
  ...
  function sortChildrenRecursively(element: UnstructuredElementWithChildren) {
    ...
    (element.children as UnstructuredElementWithChildren[]).forEach(
